...
config.lang.reopen_tab             = Item("reopen tab")
...
config.lang.search_text_tab        = Item("gee <text>")
...
class CommandRule(MappingRule):
    ...
    mapping = {
        ...
        config.lang.close_tab_n:        Key("0") + Text("%(n)d") + Key("enter/20, c-w"),
        ...
        "show groups": Key("cs-e"),
        "next group": Key("c-backtick"),
        "preev group": Key("cs-backtick"), 

        config.lang.show_tab_n:         Key("0") + Text("%(n)d") + Key("enter"),
		...
		config.lang.reopen_tab:         Key("cs-t"),
        ...
        "quick <text>": Text("'%(text)s") + Key("enter"),
        ...
        config.lang.search_text:        Key("c-k")
                                         + Text("%(text)s") + Key("enter"),
		config.lang.search_text_tab:    Key("c-t,c-k")
                                         + Text("%(text)s") + Key("enter"), 
...
gmail_context = (AppContext(title = "Gmail") | AppContext(title = "Google.com Mail")) & context
gmail_grammar = Grammar("firefox_gmail", context = gmail_context)
...
def unload():
    ...
    if gmail_grammar: gmail_grammar.unload()
    gmail_grammar = None
